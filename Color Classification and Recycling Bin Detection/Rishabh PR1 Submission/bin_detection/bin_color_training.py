...
def bin_color_train(data):
  ...
  #training data is divided by 255 to have an array belonging to [0,1]. 
  # Smaller values are efficient and fast for calculations, 
  # without affecting the accuracy of our predictions
  X = data[:,:3].astype(np.float64)/255                         
  y = data[:,3]                          #training labels
  n = len(y)                          #Total training set data points
  d = X.shape[1]                      #no. of features, {R, G, B} 
  k = np.count_nonzero(np.unique(y))  #no. of labels, y = {1, 2, 3, 4}
  ...
  for label in sorted(np.unique(y)):
    X_k = X[y==label,:]         #Observations for each label y = {1, 2, 3, 4}
